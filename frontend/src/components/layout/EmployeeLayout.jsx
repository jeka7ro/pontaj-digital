...
import { Home, PackageOpen, Wrench, AlertTriangle, Calendar, Truck } from 'lucide-react'
import { useAuthStore } from '../../store/authStore'

export default function EmployeeLayout() {
    const location = useLocation()
    const navigate = useNavigate()
    const isHome = location.pathname === '/' || location.pathname === '/my-inventory'
    const { user } = useAuthStore()
    const isLogistics = user?.role_name === 'Logistica' || user?.role_id === 'logistica' || user?.role === 'Logistica'
    ...
    return (
        <div className="flex flex-col min-h-[100dvh] bg-slate-50">
            ...
            <nav className="fixed bottom-0 left-0 right-0 bg-blue-100/40 backdrop-blur-xl border-4 border-b-0 border-white/80 px-2 py-3 flex justify-between items-center shadow-[0_-10px_25px_rgba(59,130,246,0.5)] z-50 rounded-t-3xl">
                ...
                {/* 1. Istoric (Blue) / Utilaje for Logistics */}
                {!isLogistics ? (
                    <NavLink 
                        to="/history" 
                    ...
                    >
                        ...
                        <span className="text-xs font-bold">Utilaje</span>
                    </NavLink>
                )}
                ...
                {/* 3. Acasă (Home) SAU Inventar (Magazie pt Logistică) - Glossy Glass 3D Button */}
                <div className="relative flex justify-center w-[96px]">
                    <button
                        onClick={() => isLogistics ? navigate('/my-inventory') : handleHomePress()}
                    ...
                    >
                        {isLogistics ? <Wrench className="w-10 h-10 drop-shadow-md" /> : <Home className="w-10 h-10 drop-shadow-md" />}
                    </button>
                </div>

                {/* 4. Inventar (Green/Teal) - Hiddden if logistics (since it's main button) */}
                {!isLogistics && (
                    <NavLink 
                        to="/my-inventory" 
