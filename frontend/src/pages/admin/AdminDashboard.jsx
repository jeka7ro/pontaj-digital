...
import {
    ...
    ChevronLeft, Clock, Activity, Bell, ChevronRight, Camera, Sun, Moon, Truck, Package, Briefcase, Shield, HardHat, MessageSquareWarning, BedDouble, Wallet, PackageSearch, AlertTriangle, Megaphone, Globe, Navigation, ClipboardList, CalendarDays
} from 'lucide-react'
...
export default function AdminDashboard() {
    ...
    return (
        <div className={`h-screen w-full ${darkMode ? 'bg-slate-950' : 'bg-slate-50'} flex font-sans text-slate-800 transition-colors duration-300 overflow-hidden`}>
            {/* Sidebar */}
            <aside className={`${sidebarOpen ? 'w-64' : 'w-20'} bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-white/10 text-slate-800 dark:text-white transition-all duration-300 flex flex-col z-50 relative`}>
                ...
                <div className={`h-20 flex items-center border-b border-slate-200 dark:border-white/10 shrink-0 transition-colors ${sidebarOpen ? 'px-3' : 'px-0 justify-center'}`}>
                        ...
                        {tenant ? (
                            <>
                                <div className={`flex items-center justify-center shrink-0 ${sidebarOpen ? 'w-14 h-14' : 'w-10 h-10'}`}>
                                    <img src={tenant.logo_url || "/getapp_smart_timesheet_icon.png"} alt="Tenant Logo" className="w-full h-full object-contain bg-white rounded-lg p-1" />
                                </div>
                                {sidebarOpen && (
                            ...
                            <>
                                <div className={`flex items-center justify-center shrink-0 ${sidebarOpen ? 'w-full px-2' : 'w-10 h-10'}`}>
                                    {sidebarOpen ? (
                                        <img src="/getapp_smart_timesheet_white.png" alt="GetApp Logo" className="w-full h-12 object-contain pointer-events-none select-none dark:invert-0 invert opacity-70" />
                                    ) : (
                                        <img src="/getapp_smart_timesheet_icon.png" alt="GetApp Logo" className="w-full h-full object-contain pointer-events-none select-none scale-125 dark:invert-0 invert opacity-70" />
                                    )}
                                </div>
                            </>
                        )}
                ...
                <div className="py-5 px-4 border-t border-slate-200 dark:border-white/10 shrink-0 flex flex-col items-center justify-center gap-1.5 w-full overflow-hidden">
                    ...
                    >
                        {sidebarOpen ? (
                             <img src="/getapp_smart_timesheet_white.png" alt="GetApp" className="w-[188px] h-auto object-contain opacity-70 hover:opacity-90 transition-opacity dark:invert-0 invert" />
                        ) : (
                             <img src="/getapp_smart_timesheet_icon.png" alt="GetApp Icon" className="w-10 h-10 object-contain opacity-70 hover:opacity-90 transition-opacity scale-110 dark:invert-0 invert" />
                        )}
                    </a>
                    {sidebarOpen && (
                        <a 
                            ...
                            target="_blank" 
                            rel="noreferrer" 
                            className="text-[11.5px] font-extrabold text-slate-900 dark:font-normal dark:text-slate-400 hover:text-blue-600 dark:hover:text-white transition-colors mt-0.5"
                        >
                            www.getapp.ro
                        </a>
                    )}
                    ...
                    <div className="flex items-center gap-5">
                        ...
                        <div className="flex items-center gap-2">
                           ...
                           <button
                               ...
                               className="w-7 h-7 rounded-full flex items-center justify-center text-slate-900 font-extrabold dark:font-normal dark:text-slate-400 hover:text-blue-600 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/10 transition-all duration-200"
                           >
                               {darkMode
                           ...
                           <button onClick={() => navigate('/admin/complaints')} className={`p-2.5 rounded-full transition-colors relative text-slate-300 hover:text-white hover:bg-slate-100 dark:hover:bg-white/10`} title="Sesizări Noi">
                           ...
                           <button className={`p-2.5 rounded-full transition-colors relative text-slate-300 hover:text-white hover:bg-slate-100 dark:hover:bg-white/10`} onClick={() => navigate('/admin/notifications')}>
                               <Bell className="w-5 h-5 text-slate-600 dark:text-slate-400 group-hover:text-blue-600 dark:group-hover:text-white transition-colors" />
                               {unreadCount > 0 && (
