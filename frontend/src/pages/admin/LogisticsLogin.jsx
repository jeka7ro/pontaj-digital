...
export default function LogisticsLogin() {
    ...
    useEffect(() => {
        const saved = localStorage.getItem('pontaj_logistics_saved_login')
        if (saved) {
            try {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (rememberMe) {
                localStorage.setItem('pontaj_logistics_saved_login', JSON.stringify({ email, password }))
            } else {
                localStorage.removeItem('pontaj_logistics_saved_login')
            }

            if (admin.role === 'LOGISTICS') {
                setAuth(admin, access_token)
                navigate('/logistica/warehouse')
            } else {
                logout()
                setError('Acces respins. Contul nu are drepturi de logistică.')
            }
        } catch (err) {
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div>
                         ...
                         placeholder:text-white/50"
                                placeholder="logistica@pontaj.ro"
                                required
                                autoFocus
                            />
                        </div>
                       ...
                       flex items-center justify-center gap-2 group"
                        >
                            {loading ? (
                                ...
                                <>
                                    <span>Autentificare Logistică</span>
